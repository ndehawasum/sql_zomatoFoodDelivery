...
-- Question:  Calculate and compare the order cancellation rate for each restaurant between
...
-- Question: Segment customers into 'Gold' or 'Silver' groups based on their total spending 
...
(
    ...
    SELECT 
        *,
        ...
        ROUND(EXTRACT(EPOCH FROM (d.delivery_time - o.order_time + 
        CASE WHEN d.delivery_time < o.order_time THEN INTERVAL ' 1 day' ELSE
        INTERVAL '0 day' END))/60,2) as time_deliver
    FROM orders as o
    JOIN deliveries as d
    ON o.order_id = d.order_id
    WHERE d.delivery_status = 'Delivered'
        ...
        SELECT 
            *,
            ...
            CASE 
                ...
                WHEN EXTRACT (MONTH FROM order_date) >  5 AND 
                EXTRACT (MONTH FROM order_date) < 8 THEN 'Summer'
                WHEN EXTRACT (MONTH FROM order_date) >  8 AND 
                EXTRACT (MONTH FROM order_date) < 11 THEN 'Fall'
                ELSE 'Winter'
            END as seasons
        FROM orders
) as t1
